...
uses one donor: the scaffold template (for `synthesize_empty_multirow`).
RTF prefix/suffix are hardcoded literals; phase-A is a standalone
resource file (`comment_phase_a.bin`).
